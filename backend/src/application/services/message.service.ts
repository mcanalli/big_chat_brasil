  ...
  async sendMessage(dto: SendMessageDto): Promise<MessageEntity> {
    ...
    try {
      ...
      const conversation = await this.conversationService.findOrCreate(
        client.id,
        dto.recipientPhone,
        dto.recipientName,    // 3º parâmetro (string | undefined)
        queryRunner.manager,  // 4º parâmetro (EntityManager)
      );
      ...
      // Publicação assíncrona na fila do RabbitMQ sem bloquear a resposta HTTP
